...
3. **Seed the Emulator**: While the emulator is running, import the data by running:

   ```bash
   yarn db:seed
   ```

   By default this imports the committed synthetic fixture at `scripts/synthetic-seed-data.json` — no production credentials needed. It covers every real track and course with synthetic lesson titles and content, so it's safe to commit and safe to run in Continuous Integration (CI). This is also what CI uses to run the Cypress e2e suite (see `.github/workflows/ci.yml`); if you add a course in `app/data/tracks.ts` or change how the Cypress specs depend on seeded data (e.g. a spec that needs more version history rows for a given lesson), update this fixture to match.

The emulator is now seeded and ready for local development!

##### Seeding with Production Data

To test against representative, real production content instead of the synthetic fixture (e.g. to debug something that only reproduces with real data):
...
3. **Seed the Emulator from the Backup**: While the emulator is running (see steps 1-2 above), import the downloaded production data backup by running:

   ```bash
   yarn db:seed:prod
   ```

   The emulator is now seeded with live-representative records instead of the synthetic fixture.
